...
parser = argparse.ArgumentParser(description='PyTorch RNN EncDec prediction model for time-series heart signals')
parser.add_argument('--data', type=str, default='ecg',
                    help='type of dataset required, currently only supporting ecg, could include more such as respiration')
...
def generate_output(args, epoch, model, gen_dataset, disp_uncertainty=True, startPoint=500, endPoint=350):
    if args.save_fig:
        ...
        target = preprocessing.reconstruct(gen_dataset.cpu(), TimeseriesData.mean, TimeseriesData.std)
        outSeq = preprocessing.reconstruct(outSeq, TimeseriesData.mean, TimeseriesData.std)
