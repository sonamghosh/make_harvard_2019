...
def data():
    ...
    with open(read_file_path, "r") as input_file:
        for index, line in enumerate(input_file):
            ...
            entry1 = {"x": index, "y": hr1, "z": 0.1, "size": 0.1, "color": "#ff0000"}
            entry2 = {"x": index, "y": hr2, "z": 0.1, "size": 0.1, "color": "#00ff00"}
            giantDataDump.append(entry1)
            giantDataDump.append(entry2)
            print(str(index/10) + " " + str(hr1) + " " + str(hr2))
    input_file.close()
    print("CLOSED INPUT FILE")
    write_to_file_path = "./ReactLiveChart/datas.json"
    output_file = open(write_to_file_path, "w+")
    print("OPENED OUTPUT FILE")
    print(str(giantDataDump))
    output_file.write(json.dumps(giantDataDump))
    print("Wrote TO OUTPUT FILE")
    output_file.close()
    return send_file('./ReactLiveChart/datas.json')
