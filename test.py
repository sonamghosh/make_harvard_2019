...
from anomaly_detector import get_precision_recall

parser = argparse.ArgumentParser(description='PyTorch LSTM EncDec Detection Model')
...
# Load data
###############################################################################
TimeseriesData = preprocessing.PickleDataLoad(data_type=args.data,filename=args.filename, augment_test_data=False)
...
model = model.HeartRNN(enc_inp_size=n_features,
...
try:
    ...
    for channel_idx in range(n_features):
        ...
        target = preprocessing.reconstruct(test_dataset.cpu()[:, 0, channel_idx],
                                           TimeseriesData.mean[channel_idx],
                                           TimeseriesData.std[channel_idx]).numpy()
        mean_prediction = preprocessing.reconstruct(sorted_prediction.mean(dim=1).cpu(),
                                                    TimeseriesData.mean[channel_idx],
                                                    TimeseriesData.std[channel_idx]).numpy()
        oneStep_prediction = preprocessing.reconstruct(sorted_prediction[:, -1].cpu(),
                                                       TimeseriesData.mean[channel_idx],
                                                       TimeseriesData.std[channel_idx]).numpy()
        Nstep_prediction = preprocessing.reconstruct(sorted_prediction[:, 0].cpu(),
                                                     TimeseriesData.mean[channel_idx],
                                                     TimeseriesData.std[channel_idx]).numpy()
        ...
        score = score.cpu()
        print('Anomaly scores = ', score.numpy().reshape(-1,1))
        plt.plot(score.numpy().reshape(-1,1))
        plt.show()
